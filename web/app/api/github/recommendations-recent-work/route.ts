...
export async function POST(req: Request) {
  try {
    ...
    logWithTimestamp(`Received recent work recommendation request for repository: ${repoName}`)
    ...
    // For recent work recommendations, we always generate new ones
    // Call the engine server to generate recommendations
    logWithTimestamp("Calling engine server to generate recent work recommendations")
    
    try {
      // The engine server is running on port 8000
      // Using native fetch API
      const engineResponse = await fetch("http://localhost:8000/github-recommendations-recent-work", {
        method: "POST",
        headers: {
      ...
      logWithTimestamp(`Received recent work recommendation data with ${recommendationData.recommendations?.length || 0} recommendations`)

      // Save the recommendation data to the database with a different title to distinguish it
      const saveResult = await savePaperRecommendation(
        `${repoName}-recent-work`,
        recommendationData.query,
        recommendationData
      )
      
      logWithTimestamp(`Saved recent work recommendation data to database: ${saveResult.success ? 'success' : 'failed'}`)
    ...
    logWithTimestamp(`Error in recent work recommendations API: ${error instanceof Error ? error.message : String(error)}`)
    return NextResponse.json(
